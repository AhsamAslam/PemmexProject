...
namespace Holidays.API.Queries.GetTeamHeirarchyHolidays
{
    public class GetTeamHeirarchyHolidaysQuery : IRequest<List<EmployeeHolidayDto>> { 
        public string[] employeeIdentifiers { get; set; }
        public string organizationIdentifier { get; set; }
    }
    public class GetTeamHeirarchyHolidaysQueryHandeler : IRequestHandler<GetTeamHeirarchyHolidaysQuery, List<EmployeeHolidayDto>>
    {
        private readonly IApplicationDbContext _context;
        private readonly IDateTime _dateTime;
        private readonly IMapper _mapper;
        private readonly DbContextOptionsBuilder<HolidaysContext> optionsBuilder = null;
        private readonly IHolidaySettingRepository _holidaySettingContext;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public GetTeamHeirarchyHolidaysQueryHandeler(IApplicationDbContext context, IDateTime dateTime,
            IMapper mapper, IConfiguration configuration, IHolidaySettingRepository holidaySettingContext,
            IHttpContextAccessor httpContextAccessor)
        {
            ...
            _holidaySettingContext = holidaySettingContext;
        ...
        public async Task<List<EmployeeHolidayDto>> Handle(GetTeamHeirarchyHolidaysQuery request, CancellationToken cancellationToken)
        {
            try
            {
                
                
                var settings = await _holidaySettingContext.GetOrganizationHolidaySettings(request.organizationIdentifier);
                var settingIdentifiers = settings.Select(s => s.HolidaySettingsIdentitfier).ToArray();
                if (settingIdentifiers.Length > 0)
                {
                    var holidays = await _context.CompanyToEmployeeHolidays
                    .Where(e => request.employeeIdentifiers.Contains(e.EmployeeIdentifier) && settingIdentifiers.Contains(e.HolidaySettingsIdentitfier))
                    ...
                    {
                        var s = settings.FirstOrDefault(p => p.BusinessIdentifier == e.businessIdentifier);
                        var h = await GetUsedHolidaysEmployee(e.EmployementStartDate.ToDateTime3(), s, e.EmployeeIdentifier);
                ...
                {
                    throw new Exception("No Holiday Settings are found");
                }
            }
            catch (Exception)
            {
